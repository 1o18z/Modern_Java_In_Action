...
- 즉, 시그니처란 함수 인터페이스 추상메서드의 인수와 반환값에 대한 규격? 형식? 정도로 이해하면 좋을 듯 (함수 디스크립터와 시그니처의 차이가 뭐지??)
...
- 제네릭 파라미터는 참조형 일때만 사용 가능하다. 제네릭 내부 구현때문에 어쩔 수 없는 일이다.(왜?)
